...
public class AutoAlignSwerve extends Command{
    ...
    public void execute() {
        /* Get Values, Deadband*/
        double translationVal = MathUtil.applyDeadband(limelightRangeProportional(), Constants.moveDeadband);
        double strafeVal = MathUtil.applyDeadband(strafeSup.getAsDouble(), Constants.moveDeadband);
        double rotationVal = MathUtil.applyDeadband(limelightAimKP(), Constants.rotateDeadband);
